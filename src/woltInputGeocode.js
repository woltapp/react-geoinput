...
const defaultOptions = {
  ...
  serializeGeoDestination: geoDestination => ({
    ...
    // location is empty it'll be "_.Q" object
...
const woltInputGeocode = (options = defaultOptions) => WrappedInput => class WoltInputGeocode extends Component {
  static displayName = `WoltInputGeocode(${getDisplayName(WrappedInput)})`;
  ...
  loadPredictions = (input) => {
    ...
    this.autocompleteService.getPlacePredictions({ ...mapsApiOptions, input }, (predictions, status) => {
  ...
  loadGeoDestination = () => {
    ...
    if (addressInput.value) {
      ...
      geocodeByAddress(addressInput.value).then(results => {
        ...
        this.setState({ loadingGeoDestination: false }, () => geoDestinationInput.onChange(result));
      }, () => {
        this.setState({ loadingGeoDestination: false }, () => geoDestinationInput.onChange({}));
      });
    }
  }
...
export default woltInputGeocode;
